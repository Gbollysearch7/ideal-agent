...
function DraggableBlock({ type, icon, description }: DraggableBlockProps) {
  ...
  return (
    <button
      ...
      {...listeners}
      className={cn(
        'flex cursor-grab flex-col items-center gap-2 rounded-lg border border-stone-800 bg-stone-900/50 p-3 transition-all',
...
export function DraggableBlockPalette() {
  const groupedBlocks = blockOptions.reduce(
    ...
    {} as Record<string, typeof blockOptions>
  );
